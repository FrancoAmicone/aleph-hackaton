...
dht.ready().then(() => {
  log(`DHT ok | host=${dht.host} port=${dht.port} firewalled=${dht.firewalled} bootstrapped=${dht.bootstrapped}`)
})
...
if (modo === 'cliente') {
  ...
  conn.on('error', (e) => {
    log(`❌ error: ${e.message}`)
    console.log('')
    console.log('   Si dice timeout o similar: el hole punching no pudo abrir el túnel.')
    console.log('   Probá que uno de los dos use hotspot de celular.')
  })
